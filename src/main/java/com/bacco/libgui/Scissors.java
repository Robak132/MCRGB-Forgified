...
public final class Scissors {
    ...
    public static Frame push(int x, int y, int width, int height) {
        try (Frame frame = new Frame(x, y, width, height)) {
            STACK.push(frame);
            refreshScissors();
            return frame;
        } catch (Exception e) {
            throw new IllegalStateException("Unexpected exception when pushing a scissor frame", e);
        }
    }
    ...
    public static Frame pop() {
        if (STACK.isEmpty()) throw new IllegalStateException("No scissors on the stack!");

        try (Frame frame = STACK.pop()) {
            refreshScissors();
            return frame;
        } catch (Exception e) {
            throw new IllegalStateException("Unexpected exception when popping a scissor frame", e);
        }
    }
